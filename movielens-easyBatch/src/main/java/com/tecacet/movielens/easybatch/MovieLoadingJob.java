...
public class MovieLoadingJob {
    ...
    class DelegatingRecordMapper implements RecordMapper<StringRecord, Record<Movie>> {
        ...
        private DelimitedRecordMapper recordMapper = new DelimitedRecordMapper(Movie.class, new Integer[] { 0, 1, 2, 3, 4 },
    ...
    public JobReport readMovies() throws Exception {
        Job job = buildJob();
    ...
    private Job buildJob() throws FileNotFoundException {
        ...
        RecordMapper recordMapper = new DelegatingRecordMapper();
        Job job = JobBuilder.aNewJob().jmxMode(true).reader(new FlatFileRecordReader(new File(MOVIE_FILENAME))).mapper(recordMapper)
